define([
    ...
    var ReserveControl = CFS.View.extend({
        ...
        initialize: function () {
            ...
            var isMobile = isTouch && !isDesktop;

            if (this.$el.find('#syos').length !== 0 && !isMobile) {
                this.openPanel = "syos";
            }
            else {
                this.openPanel = "best";
        ...
        parseCalendarInfo: function (response) {
            var calInfo = response.d;
            console.log(calendarInfo);
            if (calInfo == null)
                calInfo = calendarInfo;
            calInfo.Performances = calendarInfo.Performances;
            calInfo.Performances.forEach(function(perf) {
                perf["LongDateString"] = createLongDate(perf.Date);
            });
            console.log("Calendar info recieved: ", calInfo);
            this.calendar.parseInfo(calInfo);
            this.syncView();
            return this;
        },
        ...
        syncText: function () {
            ...
            if (typeof performanceInfo !== 'undefined') {
                var time = performanceInfo.get('Time');
                if (time === "11:59 PM") {
                    time = "Midnight";
                }
                this.dateText.html(performanceInfo.get('LongDateString') + " " + time);
            }
            return this;
        },
...
function createLongDate(dateStr){
