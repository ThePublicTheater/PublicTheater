...
(function ($) {
        ...
        init: function (options) {
            ...
            return this.each(function () {
                ...
                if (!data) {
                    ...
                    $sliderContainer.width(originalWidth + 5);
                    ...
                    $(window).resize( _.debounce(function (e,t) {
                        showHideControlsResize($slider);
                    }, 250));
                    $slider.bind('resize', function (e, t) { showHideControlsResize($slider); });
    ...
    // Private functions
    //
    function showHideControlsResize($slider) {
        var outer = $slider;
        var rightmost = outer.children().children().last();
        if (outer.width() >= rightmost.position().left+rightmost.outerWidth()) {
            $slider.next().filter('.controls').hide();
        } else {
            $slider.next().filter('.controls').show();
        }
    }
